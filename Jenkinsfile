node{
    ...
    stage('Deploy to non-prod'){
        load 'dataproc-job-orchestration/variables.groovy'
        sh '''gsutil cp gs://centralised-job-artifact/${env.ARTIFACT} gs://dataproc-staging-artifact
        gsutil cp dataproc-job-orchestration/dataproc_job_submission.py gs://us-central1-centralised-dat-237092eb-bucket/dags
    ...
    stage('Deploy to prod'){
        ...
        sed -i \'s/non_prod/prod/\' dataproc-job-orchestration/dataproc_job_submission.py
        gsutil cp dataproc-job-orchestration/dataproc_job_submission.py gs://us-central1-centralised-dat-237092eb-bucket/dags
        gcloud composer environments run centralised-dataproc-job-orchestration --location us-central1 trigger_dag -- composer_hadoop'''
    }
}